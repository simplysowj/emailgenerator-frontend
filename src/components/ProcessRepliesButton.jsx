...
const ProcessRepliesButton = ({ campaignId }) => {
  ...
  const fetchStats = async () => {
    try {
      const response = await axios.get(`https://email-backend-bee9bjdec6gkhuf3.eastus2-01.azurewebsites.net/api/campaigns/${campaignId}/reply_stats/`);
  ...
  const handleProcessReplies = async () => {
    ...
    setError(null);
    try {
      const response = await axios.post(`https://email-backend-bee9bjdec6gkhuf3.eastus2-01.azurewebsites.net/api/campaigns/${campaignId}/process_replies/`);
